...
def main():
    ...
    # Define the *actual* output directory path, including the SNR
    sample_path = os.path.join(args.results_path, 'samples' + "_SNR" + str(args.snr))
    # Create that specific directory
    os.makedirs(sample_path, exist_ok=True)
    ...
    for i, (batch, cond) in enumerate(data):
        ...
        model_kwargs = preprocess_input(args, cond, num_classes=args.num_classes, one_hot_label=args.one_hot_label)
        # model_kwargs = preprocess_input_FDS(args, cond, num_classes=args.num_classes, one_hot_label=args.one_hot_label)
        ...
        for j in range(sample.shape[0]):
            ...
            # --- START OF FIX ---
            # Get the base filename without extension in an OS-safe way
            original_path = cond['path'][j]
            base_filename = os.path.basename(original_path).split('.')[0]

            # Use the clean base_filename to save all images
            tv.utils.save_image(image[j], os.path.join(image_path, base_filename + '.png'))
            # tv.utils.save_image(sample[j], os.path.join(sample_path + "_SNR" + str(args.snr), base_filename + '_SNR' + str(args.snr) + '_pool' + str(args.pool) + '.png'))
            tv.utils.save_image(sample[j], os.path.join(sample_path, base_filename + '_SNR' + str(args.snr) + '_pool' + str(args.pool) + '.png'))
            tv.utils.save_image(label[j], os.path.join(label_path, base_filename + '.png'))
            # --- END OF FIX ---
...
#     print("label map shape:", label_map.shape)
    ...
    if 'instance' in data:
        ...
        # input_semantics = th.cat((input_semantics, instance_edge_map), dim=1)
        input_semantics = th.cat((input_semantics, instance_edge_map.squeeze(0)), dim=0)
    ...
    input_semantics = input_semantics[map_to_be_preserved]
