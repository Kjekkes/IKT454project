...
class UNetModelSPADE(UNetModel):
    """
    A UNetModel that uses SPADE normalization in the up-sampling path.
    """
    def __init__(
        self,
        image_size,
        ...
        self.output_blocks = nn.ModuleList([])
        for i, (level, mult) in list(enumerate(channel_mult))[::-1]:
...
class UNetModelEncOnly(UNetModelSPADE):
    """
    A UNetModel that provides the encoder functionality only.
    """
