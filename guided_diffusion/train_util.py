...
class TrainLoop:
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            micro = batch[i : i + self.microbatch].to("mps")
            micro_cond = {}
            for k, v in cond.items():
                # Get the tensor for the current microbatch
                micro_v = v[i : i + self.microbatch]

                # Experiment 1: Additive Gaussian Noise
                if k == 'y' and self.cond_noise_std > 0.0:
                    noise = torch.randn_like(micro_v) * self.cond_noise_std
                    micro_v = micro_v + noise

                # Experiment 2: Information Erasure (Dropout)
                elif k == 'y' and self.cond_dropout_rate > 0.0:
                    # Create a binary mask
                    # (torch.rand > p) creates a mask where p% of elements are 0 (dropped)
                    mask = (torch.rand_like(micro_v, dtype=torch.float32) > self.cond_dropout_rate).float()
                    micro_v = micro_v * mask # Apply mask
                    
                micro_cond[k] = micro_v.to("mps")
            last_batch = (i + self.microbatch) >= batch.shape[0]
            # t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())
            # t, weights = self.schedule_sampler.sample(micro.shape[0], "cuda")
            t, weights = self.schedule_sampler.sample(micro.shape[0], "mps")
